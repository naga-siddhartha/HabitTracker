...
struct HabitTimelineProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> HabitEntry {
        HabitEntry(date: .now, habits: [], completedCount: 2, totalCount: 5)
    }
    ...
    private func fetchEntry() -> HabitEntry {
        ...
        guard let habits = try? context.fetch(descriptor) else {
            return HabitEntry(date: today, habits: [], completedCount: 0, totalCount: 0)
        }
...
#Preview("Small", as: .systemSmall) {
    ...
    HabitEntry(date: .now, habits: [], completedCount: 3, totalCount: 5)
}
